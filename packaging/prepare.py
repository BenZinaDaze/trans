...
def main() -> None:
    ...
    match = re.search(r"\bproject\s*\(\s*tran\s+VERSION\s+(\d+\.\d+\.\d+)\s", cmake)
    if not match:
        parser.error("CMakeLists.txt must declare project(tran VERSION X.Y.Z ...)")
    ...
    archive = output / f"tran-{version}.tar.gz"
    with archive.open("wb") as stream:
        subprocess.run(["git", "archive", "--format=tar.gz", f"--prefix=tran-{version}/", "HEAD"],
    ...
    print(f"Prepared tran {version} from {git('rev-parse', 'HEAD').decode().strip()}")
