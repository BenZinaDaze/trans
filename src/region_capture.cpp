...
X11RegionScreenshotJob::X11RegionScreenshotJob(QObject *owner) : ScreenshotJob(owner)
{
    ...
    connect(&m_timer, &QTimer::timeout, this, [this] { fail(ErrorCode::Timeout, QStringLiteral("截图请求超时，请重试。")); });
...
void X11RegionScreenshotJob::start()
{
    ...
    if (QGuiApplication::platformName() != "xcb") {
        fail(ErrorCode::Configuration, QStringLiteral("当前框选后端需要 X11 会话。")); return;
    }
    ...
    for (auto *screen : screens) {
        ...
        if (image.isNull()) {
            fail(ErrorCode::Network, QStringLiteral("无法读取屏幕图像，请重新截图。")); return;
        }
    ...
    if (m_overlays.isEmpty()) {
        fail(ErrorCode::Network, QStringLiteral("未找到可截图的屏幕。")); return;
    }
...
void X11RegionScreenshotJob::fail(ErrorCode code, const QString &message)
{
    if (m_done) return;
    m_done = true;
    m_timer.stop();
...
void X11RegionScreenshotJob::cancel() { fail(ErrorCode::Cancelled, QStringLiteral("截图已取消。")); }

ScreenshotJob *createScreenshotJob(QObject *owner)
{
    if (QGuiApplication::platformName() == "xcb") return new X11RegionScreenshotJob(owner);
    return new PortalScreenshotJob(owner);
}
} // namespace Trans
